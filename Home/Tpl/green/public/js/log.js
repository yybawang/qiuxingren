$(function(){
	...
	$("#register_form").submit(function(){
		...
		$.post(url,{email:email,user_name:user_name,password:password,repassword:repassword},function(msg){
			if(msg.status == 1){
				...
				if($t.closest('.pop_big').length > 0){
					$('.user_bar .login').siblings('.logout').css('display','none').siblings('.login').fadeIn(300).find('.username').text(msg.data.user_name);
					$t.closest('.pop_big').fadeOut(100);
				}else{
	...
	$('#go_out').click(function(){
		$.post(GROUP+'/user/logout_ajax');
		$('.user_bar .login').css('display','none').find('.username').text('').closest('.login').siblings('.logout').fadeIn(300);
		return false;
	});
});
