...
<div class="audit">
	<div class="auditer">
		<?php if($content == null): ?><div class="null">
				悲剧，被审核完了
			</div>
		<?php else: ?>
		<div class="content">
			<div class="ithink">我觉得吧，这件糗事....</div>
			<div class="text"><?php echo ($content["content"]); ?>
			<?php if($content["image"] != null): if(is_array($content["image"])): foreach($content["image"] as $key=>$img): ?><div><img width="300" src="<?php echo ($img["image_link"]); ?>" /></div><?php endforeach; endif; endif; ?>
			</div>
		</div>
		<div class="option">
			<a action="yes" a_id="<?php echo ($content["article_id"]); ?>"><span class="yes"></span><p>朕准了</p></a>
			<a action="no" a_id="<?php echo ($content["article_id"]); ?>"><span class="no"></span><p>推下去斩了</p></a>
			<a action="pass" a_id="<?php echo ($content["article_id"]); ?>"><span class="pass"></span><p>退朝,下次再议</p></a>
		</div><?php endif; ?>
	</div>
</div>
<script>
