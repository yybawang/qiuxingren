	...
	if(action == 'comment'){
		...
		<div class="right">
			...
			<?php if(!empty($content)): ?><!-- text循环 -->
				...
				<div class="single"><a title="按住此按钮删除" class="del_message option a" a_id="<?php echo ($data["message_id"]); ?>" href="javascript:;">删除</a></div>
					<!-- <img width="20" height="20" src="__PUBLIC__<?php echo ($data["from"]["pic"]); ?>" /> -->
					<a class="a user_info" u_id="<?php echo ($data["from"]["user_id"]); ?>" href="__URL__/user_info"><?php echo ($data["from"]["user_name"]); ?></a>：<?php echo ($data["content"]); ?>
					<span class="date"><?php echo (date("Y-m-d H:i:s",$data["add_time"])); ?></span>
...
<script>
	//划过显示用户信息
	user_info({
		dom : '.text a.user_info',
		public_dir : '__PUBLIC__',
		post_url : '__GROUP__/UserCenter/user_info_ajax'
	});
	del_article({
		post_url : '__GROUP__/Usercenter/del_message_ajax',
		dom : '.del_message'
	});
</script>
<div class="foot">
