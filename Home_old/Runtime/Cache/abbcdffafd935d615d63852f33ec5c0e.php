...
$(function(){
	...
	if(action == 'comment'){
		...
		<div class="right">
			<div class="righter">
			<?php if(!empty($content)): ?><!-- text循环 -->
				<?php if(is_array($content)): foreach($content as $key=>$data): ?><div class="text">
					<div class="single"><a class="option a" target="_blank" href="__GROUP__/Index/single/article/<?php echo ($data["article_id"]); ?>">查看详细</a><a title="按住此按钮删除文章" class="del_article option a" a_id="<?php echo ($data["article_id"]); ?>" href="javascript:;">删除</a></div>

					<?php echo ($data["content"]); ?>
					<p></p>
					<?php if(data.image != null): if(is_array($data["image"])): foreach($data["image"] as $key=>$img): ?><img class="article_image" src="__PUBLIC__<?php echo ($img["image_link"]); ?>"><br /><?php endforeach; endif; endif; ?>
					
					<div class="status">
					<?php if($data["is_show"] == '1'): ?><span class="yes">已通过</span>
					<?php elseif($data["is_show"] == '-1'): ?>
						<span class="no">未通过</span>
					<?php else: ?>
						<span class="pass">待审核</span><?php endif; ?>
					</div>
					<div class="L_bar">
						<ul>
