...
<div class="user">
	<div class="ucenter">
		<div class="left">
	<div class="user_info">
		<a title="个人中心" href="__URL__/center"><img src="__PUBLIC__<?php echo ($user["pic"]); ?>" /></a>
		<p><?php echo ($user["user_name"]); ?></p>
		<ul>
			<li><a href="__URL__/center">文章</a></li>
			<li><a href="__URL__/followers">粉丝</a></li>
			<li><a href="__URL__/followings">关注</a></li>
			<li><a href="__URL__/message">留言/小纸条</a></li>
			<li><a href="__URL__/comment">评论</a></li>
		</ul>
	</div>
	<div class="option">
		<ul>
			<li><a title="修改头像和基本信息" href="__URL__/update">修改资料</a></li>
			<li><a title="修改密码" href="__URL__/repwd">修改密码</a></li>
		</ul>
	</div>
...
$(function(){
	var action = '<?php echo (ACTION_NAME); ?>';
	if(action == 'center'){
		$('.user_info li:eq(0)').addClass("active");
	}
	if(action == 'followers'){
		$('.user_info li:eq(1)').addClass("active");
	}
	if(action == 'followings'){
		$('.user_info li:eq(2)').addClass("active");
	}
	if(action == 'message'){
		$('.user_info li:eq(3)').addClass("active");
	}
	if(action == 'comment'){
		$('.user_info li:eq(4)').addClass("active");
	}
});
</script>
		<div class="right">
			<div class="righter">
			<?php if(!empty($comment)): ?><!-- text循环 -->
				<?php if(is_array($comment)): foreach($comment as $key=>$data): ?><div class="text">
					<div class="single"><a class="option a" target="_blank" href="__GROUP__/Index/single/article/<?php echo ($data["article_id"]); ?>">查看详细...</a></div>
					
					<?php echo ($data["content"]); ?>
					<p></p>
					<?php if($data["image"] != null): if(is_array($data["image"])): foreach($data["image"] as $key=>$img): if($data["former_id"] > 0): ?><img src="<?php echo ($img["image_link"]); ?>" />
							<?php else: ?>
								<img src="__PUBLIC__<?php echo ($img["image_link"]); ?>" /><?php endif; endforeach; endif; endif; ?>
					<?php if(is_array($data["comments"])): foreach($data["comments"] as $key=>$comments): ?><div class="comment_comment"><ul><li class="content"><?php echo ($comments["comment_content"]); ?></li><li class="time"><?php echo (date("Y-m-d H:i:s",$comments["comment_time"])); ?></li><li class="count"><?php echo ($key+1); ?></li></ul></div><?php endforeach; endif; ?>
					
				</div><?php endforeach; endif; ?>
			<?php else: ?>
				<p class="uc_empty">空空如也哦~~~</p><?php endif; ?>
			</div>
		</div>
	</div>
</div>
