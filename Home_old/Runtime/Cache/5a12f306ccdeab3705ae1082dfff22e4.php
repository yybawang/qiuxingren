...
<div id="timeout"></div>
<?php if(is_array($content)): foreach($content as $key=>$vo): echo ($key+1); ?>---原ID：<?php echo ($vo["former_id"]); ?>---原用户ID：<?php echo ($vo["former_user_id"]); ?>---原用户名：<?php echo ($vo["former_user_name"]); ?><br /><?php echo ($vo["content"]); ?><p />
		<?php if($vo["image_link"] != null): echo ($vo["image_link"]); endif; endforeach; endif; ?>
<script>
var timeout = parseInt("<?php echo ($timeout); ?>");
jishi(timeout*60/1000);
function myrefresh() 
{ 
	window.location.reload(); 
}
function jishi(time){
	$("#timeout").text("剩余秒数约:"+time);
	setTimeout(function(){jishi(time-1)},1000);
}
setTimeout('myrefresh()',timeout*60); //指定三分钟刷新一次
</script>
<div class="foot">
	<div class="friend">
