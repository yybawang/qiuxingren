...
$(function(){
	...
	if(action == 'comment'){
		...
		<div class="right">
			<div class="righter">
				<form action="__URL__/change" enctype="multipart/form-data" method="post">
					
					<div class="email">邮&emsp;箱：<span><?php echo ($user["email"]); ?></span></div>
					<div class="user_name">用户名：<input type="text" name="user_name" value="<?php echo ($user["user_name"]); ?>" /></div>
					<div class="pic">头&emsp;像
					<?php if($user.pic): ?><input type="file" name="pic" /><br />
						<img width="150" src="__PUBLIC__/<?php echo ($user["pic"]); ?>" />
					<?php else: ?>
						<input type="file" name="pic" /><?php endif; ?>
					</div>
					<div class="sub"><input id="sub" type="submit" value="提交" /></div>
				</form>
			</div>
		</div>
	</div>
